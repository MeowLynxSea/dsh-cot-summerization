...
function reasoningBlocks(index: number, text: string): StreamChunk[] {
  return [
 ...
 */
async function replacementReasoning(
  rawCoT: string,
  cfg: ResolvedCotSummarizerConfig,
  summarize: (raw: string, cfg: ResolvedCotSummarizerConfig, signal: AbortSignal | undefined) => Promise<string>,
  callerSignal: AbortSignal | undefined,
  log: (message: string, ...args: unknown[]) => void,
): Promise<string | undefined> {
  const trimmed = rawCoT.trim()
  if (trimmed === '') return undefined
  if (callerSignal?.aborted === true) return undefined
  if (trimmed.length < cfg.minReasoningChars) return trimmed
  try {
    ...
    if (cfg.onError === 'pass-through') return trimmed
