...
/** One tracked model stream per session: its raw capture plus the step identity seen in its chunks. */
 ...
 * execution.
 */
export function createRawHistoryRestorer(log: RestorerLog = () => {}): RawHistoryRestorer {
  const trackers = new Map<string, RawTracker>()
  return {
    ...
    handleSessionEvent(session, event) {
      ...
      if (tracker.turn === undefined || tracker.turn !== event.data.turn || tracker.step !== event.data.step) return
      trackers.delete(key)
      ...
      queueMicrotask(() => {
        try {
          ...
          log('cot-summarizer: restoring the raw reasoning on the model-visible surface failed: %s',
