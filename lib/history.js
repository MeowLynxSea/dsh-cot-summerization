...
export function captureReasoningStart(capture) {
    capture.rawReasoning.push('');
}
/** Append one raw reasoning delta to the current upstream reasoning block. */
export function captureReasoningDelta(capture, text) {
    if (capture.rawReasoning.length === 0)
        capture.rawReasoning.push('');
