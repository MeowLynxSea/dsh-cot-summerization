...
export async function* transformCoTStream(upstream, cfg, summarize, callerSignal, log = () => { }, capture = undefined, now = Date.now) {
    ...
    const preReplyDeadlinePassed = () => reasoningDone
    ...
    async function* streamReasoningBlock() {
        ...
        if (cfg.streamReasoningBlock && pending !== undefined && !pendingSettled) {
            ...
            try {
                ...
                const winner = await Promise.race([pending.then(() => 'settled'), timeout]);
                if (winner === 'settled')
                    yield* foldPending();
            }
            finally {
    ...
    const isReplyChunk = (c) => c.type === 'text-delta' || c.type === 'tool-call-delta'
    ...
    for await (const chunk of upstream) {
        ...
        if (isReplyChunk(chunk) && sawAnyReasoning && !reasoningDone)
            replyInterleaved = true;
        ...
        switch (chunk.type) {
            ...
            case 'finish': {
                ...
                if (!aborted) {
                    ...
                    if (pending !== undefined && !summaryClosed)
                        yield* foldPending();
                    const trimmed = rawCoT.trim();
