...
export async function* transformCoTStream(upstream, cfg, summarize, callerSignal, log = () => { }) {
    ...
    function* applyResult(result) {
        ...
        if (result.startsWith(emitted)) {
            ...
            if (common >= Math.max(16, Math.floor(emitted.length / 2)))
                tail = result.slice(common);
        }
        if (tail === '') {
            stalled += 1;
            return;
    ...
    const maybeFirePartial = () => {
        if (pending !== undefined || finalFired || !cfg.incremental)
            return;
        if (stalled >= 2 || emitted.length >= cfg.maxSummaryChars)
            return;
    ...
    for await (const chunk of upstream) {
        ...
        if (pendingSettled) {
            ...
            if (reasoningDone && !finalFired && !rawShown && pending === undefined) {
                ...
                if (trimmed !== '' && trimmed.length >= cfg.minReasoningChars)
                    fire(true);
            }
        }
        switch (chunk.type) {
