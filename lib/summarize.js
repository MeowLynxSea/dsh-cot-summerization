 ...
 */
export function escapeDelimitedText(text) {
    return text
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;');
 ...
 */
export function parseSummaryPayload(result) {
    ...
    if (tolerant?.[1] !== undefined) {
        const escapedValue = tolerant[1]
            ...
            .replace(/\t/g, '\\t');
        try {
            const summary = JSON.parse(`"${escapedValue}"`);
            if (typeof summary === 'string' && summary.trim() !== '')
                return summary.trim();
        }
        catch {
